...
**Read every doc this dimension consults at the PR head, not the working tree.** The evaluator's cwd is the repo root, which holds the *base-branch* copy of the docs; a PR that legitimately ships a doc edit (e.g. an "Update doc first" deviation) must be judged against its *own* post-edit doc, or the dimension flags a violation the PR already resolved. Read each doc via `git show <HEAD_SHA>:<path>` (`HEAD_SHA` from §5.1) — run `git fetch --quiet origin <headRefName>` once first, since the §5.5.0 worktree may not exist on the cache-hit / CI-green / skipped-gate paths that reach §6. A doc absent at `HEAD_SHA` is simply not present.
...
- **`## Doc grounding` section present and specific** → verify it: read the cited PRD / architecture / constitution / CLAUDE.md sections at the PR head (`git show <HEAD_SHA>:<path>`) and confirm they actually say what the section claims and that the diff honors them.
- **Section absent, or too vague to corroborate** → do the grounding work yourself. Independently check the diff against the project's canonical docs at the PR head, read-if-present via `git show <HEAD_SHA>:<path>`: `docs/prd.md`, `docs/architecture.md`, `docs/constitution.md`, `CLAUDE.md`. **Bound this exactly like the *Plan adherence* dimension does** — don't load the full diff; enumerate changed files with `grep '^diff --git ' <diff_path>`, map them to the doc sections they plausibly implicate, then compare only those diff hunk ranges (from `<diff_path>`) against only those doc sections (`git show <HEAD_SHA>:<doc>`). A missing section is *not itself a gap* — it only raises the depth of the check you run.

**`## Follow-ups` re-adjudication.** If the PR body carries a `## Follow-ups` section listing filed deferrals (e.g. "pagination → #45"), do **not** trust a filed follow-up as a settled deferral — a follow-up the resolver filed (or hand-classified on an integration PR) can be laundering an in-scope grounding violation past this gate. Re-check each entry: read the doc it implicates at the PR head (`git show <HEAD_SHA>:<path>`) plus the issue's / parent epic's `## Out of scope`, and judge:
  - **In-scope documented-constraint violation** — the cited doc *requires* the deferred behaviour (an invariant / MUST / "no X" rule) and the `## Out of scope` does not exclude it → **soft-reject** (`--comment`), quoting the doc section and the deferred-to issue number. A filed follow-up does **not** make it acceptable: deferring would land a non-compliant diff with a fix-later ticket behind it, which the in-scope grounding rule forbids. The severity axis is "an in-scope documented constraint is violated" — not which doc it lives in. As with this dimension's general verdict threshold, the soft-reject fires on a **clear** in-scope violation; a genuinely ambiguous case (is it even a violation? is it truly in-scope?) follows the flag-don't-block rule rather than hard-blocking on a maybe.
  - **Genuinely out-of-scope / future work** — the doc excludes it, or it's polish the docs don't require (e.g. pagination *tuning*, an a11y nit) → note it in the review body, do not block.

  **The filed issue is not deleted.** Its *existence* is fine — it describes real required work. What's rejected is its use as a deferral that waves the merge through. On the soft-reject the resolver re-enters (§11 flips the PR to draft), and that issue becomes the **blocking work item** its re-plan / story route points at — the PR doesn't merge until the work lands.
...
- **A `## Follow-ups` entry the doc-grounding dimension judged an in-scope documented-constraint violation** (§6's `## Follow-ups` re-adjudication). A filed deferral does not earn a pass — cite the doc section and the deferred-to issue number; the resolver re-enters and the deferral becomes blocking work.
...
### 13a. File residual non-blocking follow-ups (any merged PR)

Runs for **all** PR types — story, standard, *and* epic integration — on the merge path; this is **not** a sub-step of the story-only §13. Skip it on the "merge did not run" branch (§14), like §13.

After the merge, the evaluator files the **residual non-blocking** work this run is responsible for tracking: a finding its own review surfaced, or a `## Follow-ups` *Procedural notes* item, that warrants an issue and isn't already filed. **De-duplicate** against issues already linked in the PR body's `## Follow-ups` — its *Filed* entries are already issues, so never re-file them. In-scope grounding violations never reach this step: they soft-reject pre-merge (§6/§7), which takes the "merge did not run" branch.

File each via the shared drafter-proxy protocol in [`../_shared/follow-up-filing.md`](../_shared/follow-up-filing.md) — one `general-purpose` sub-agent per item, type from the existing `bug | incomplete-feature | deferred-test | revise-existing` enum, parent reference = this PR + the issue (+ the parent epic for a story). After filing, post the URLs as a brief PR comment so they're durable. If there is nothing residual to file, say nothing.
...
The §6 `## Follow-ups` re-adjudication is **load-bearing here** — integration is the final accumulation point, where the resolver may have skipped its §10 editing sub-agent and hand-classified review findings as follow-ups. A deferral that maps to an in-scope grounding invariant or an epic-level `## Definition of done` bullet blocks the integration: soft-reject and name the violated doc section / epic DoD bullet plus the deferred-to issue. The epic is not integration-ready until that work lands on the epic branch.
