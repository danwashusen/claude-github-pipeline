...
the classification, the grounding, the deviation/decision gates, the drafting, the review verdicts,
and the handoff `Why:`.
...
labels/`blocked_by`/`blocking`), `vector` (`type` × `mode` × `plan_ref_row`), `suggested_playbook`,
`plan_ref` (a **bare** branch name), `plan` (present/SHA/comment-id/url — a present plan is the
**revise** trigger), `research` (dossier present + staged path), `grounding_docs` (which grounding
docs exist at `plan_ref`), `open_questions` + `open_question_candidates` (the Bug (a) tracker search),
`read_workspaces.grounding` (the ONE read workspace this skill gets — detached at `plan_ref`, with the
`sha` the footer records), `epic`/`story` facts (stories + their state; the parent epic + its plan +
delivery log), `revise` (prior-plan SHA vs grounding SHA + any open PR's `## Phase tracker`),
`sections` (spilled issue-body/thread/plan-marker paths), and `attention`. Consume every fact as
**data** — never re-derive the issue type, the mode, the `plan_ref`, the plan-ref row, or the tracker
...
Every playbook opens by reading the shared spine `playbooks/plan-spine.md` (classify → ground at the
read workspace → deviation/decision gates → draft against the schema → pre-flight hedge sweep → verify
loop → show → persist plan + pointer). The routed playbook supplies only what **differs in actions**:
the schema sections it fills, the reviewer dimension set it passes, its pre-draft reconnaissance, and
its handoff shape. Type differences the spine consumes (`plan_ref`, the dimension set, which schema
sections) are **facts / values**, never branches.
...
- **Grounding is read-only, one workspace.** The planner never gets a work workspace and writes no
  ...
  `sha` is the footer's `@<short-sha>` (`architecture.md §6` "no ref arithmetic in prompts").
...
- **Successful write is self-confirming.** A zero exit with a URL *is* the confirmation; never re-read
  the comment to check it landed.
- **Gates only for genuine decisions** (per [`../_shared/asking-the-user.md`](../_shared/asking-the-user.md)):
  latest-decision-direction, external sources, the deviation gate, the decision gate, review-notes
  disposition, show-before-post (opt-in), revise reconciliation. Never to confirm a fact prep derived.
...
- **A tracked open question is a human's call — never resolved from precedent or the decision gate.**
  Record it in the plan's `## Open questions`; if the whole plan is gated, re-route rather than post a
  hollow plan.
...
or answer-the-question; revise refreshed). Fill the snapshot from data in hand; the `Next:` action and
`Why:` line are judgment. The `**Open questions:**` line renders in **every** shape whose posted plan
carries an `## Open questions` section — including a composite epic+story session — never dropped
because the structural shape matched a different example first. Next-command skills are namespaced
`/github-pipeline:<name>` (`resolver`, `drafter`, `researcher`, `planner`). A re-route points `Next:`
at a prior skill but does **not** invoke it via the `Skill` tool — the handoff is the only signal; the
