...
- **Writer — `evaluator` (sole writer).** When it merges a story PR it appends an entry recording what that story delivered, creating the comment lazily on the first story merge. Recording **every** story here — including the last — is what keeps the log complete.
...
Go through the single write path — never hand-roll `gh` (a raw `gh api` / `Write` + `gh` assembly re-opens the #626/#627 empty-body race). Stage the full updated body (marker line first) to the session's scratch dir, then:
