...
**A slice is refused, not silently promoted.** `workspace-open` and the `planner` classify the target's native parent (`branching.classify_parent` — one `--json labels,title` read, run only when no `epic/<parent>-*` branch already proved epic-ness) and raise `TARGET_IS_SLICE` when it comes back a non-epic: opening a workspace would give the slice the branch and PR that are precisely what it must not have, and planning it standalone would author a decomposition competing with its parent's. Classification is lexical, so an epic carrying neither an `epic` label nor an `Epic:` title prefix reads as a non-epic — each card therefore offers relabelling that parent, or re-parenting the target, and re-running. Those are external remedies like every other card's; neither prep carries a proceed-anyway override, and an option offering one would raise the identical card next invocation. The refusal is also suppressed entirely once the target has an open PR of its **own**: the promotion already happened, and a card cannot unwind it. The **resolver** deliberately does not carry the same refusal: a slice with no workspace opened for it fails the resolver's own attach assertion first, so the gate upstream is the one that matters.
...
**The slice edge has no checklist fallback and never will.** The two-tier read above exists only because epics predate the native relation; slices do not, so `sub_issues` is the whole story at that level and a `stories_source`-style flag has no slice analogue. For the same reason, no `## Slices` section is ever written to a parent body, and the **cut itself** never edits the parent — slice detail lives only in slice bodies. (The slicer does make two gated parent-body writes at epic altitude, neither of them part of a cut: the promotion rewrite that turns an issue into an Epic, and reconciling a legacy `## Stories` checklist, which for those entries is still their only record. Both sit behind their own explicit confirmation.)
