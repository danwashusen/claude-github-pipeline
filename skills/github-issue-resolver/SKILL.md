...
Reusable procedures the flows (§1–§12, Epic, Story) invoke by ID — each carries a stable §P-ID, cited as "per §P2" the way the flows already cite "per §10.6".
...
§7 baseline runs the project's static-checks block. **§8 (after the first round of code changes, before the first push) and every §10.6 iteration (after addressing review feedback, before the next push)** run the static-checks block followed by a Claude-selected set of test suites scoped to the diff vs the integration target — this is the **pre-push verification gate** detailed in §P3.2 and §P4. Both gates are mandatory before a push; never push code without one of them having run green. The full canonical suite (every unit test + every integration test) is **not** run inside this skill at any gate. The point of this design is fast feedback during development: a one-module edit should run that module's tests, not a 10-minute integration suite.
...
#### §P3.4 Test execution — verbose-build-log delegation

If the resolved command begins with `xcodebuild` (or invokes a wrapper that runs `xcodebuild`, like `./scripts/xcb.sh`), delegate execution to the `apple-platform-build-tools:builder` subagent. The subagent absorbs the verbose build log and returns terse pass/fail plus the first error if any — keeping conversation context clean. Non-xcodebuild commands (e.g., `bin/rails test`, `pytest`, `go test`) run inline by default — their output is usually compact. But honour the repo's own CLAUDE.md operating guidance and the command's actual behaviour: for a run flagged as slow or noisy (a system/feature-test suite, a verbose integration run), apply the background-and-capture discipline `references/epic-flow.md` uses for the full canonical suite — `run_in_background: true`, tee to a log, read back the summary — instead of streaming a large log into context. **Exception:** the *full* canonical suite in the epic-baseline / bootstrap / post-rectification flow is **not** delegated to the builder — it is a 15–30 min run owned by the main loop; see "Running the full canonical suite".
...
**Epic-only — relocated to [references/epic-flow.md](references/epic-flow.md).** The full-canonical-suite runbook (which command; `build-once` / `retry-without-rebuild` re-run discipline; main-loop-background-bash rule; absolute paths) is invoked solely by the epic-baseline / bootstrap / post-rectification flows, which force-`Read` `epic-flow.md` before running it. It is **not** run at the §8/§10.6 story gates.
...
Follow-up items surface at four moments — §7 baseline detours, the retry-ladder's escalation option 2, §10.4 reviewer-routed deferrals, and §11 summary cleanup. The registry has five fields (`type`, `title hint`, `description`, `parent reference`, `urgency`). Filing decision rule: trackable work → file as an issue via the drafter-proxy sub-agent protocol; procedural / informational notes → capture in the PR body or §11 summary instead. Urgency `file-now` items must land before the iteration's push (so `// TODO(#NNN)` markers and skip-annotation reasons (`XCTSkip(...)`, `skip "..."`) reference real issue numbers); urgency `file-at-checkpoint` items batch at end-of-§10 before §11 fires. Every filed issue routes through `github-issue-drafter` (PRD-grounded, sub-agent-reviewed) — no hand-crafted `gh issue create` bodies. After filing, weave the URL into the code's TODO / skip markers, the PR body's `## Follow-ups` section, and §11's summary.

**See [`references/follow-up-tracking.md`](references/follow-up-tracking.md) for the full registry schema, filing-vs-capturing rule, hybrid timing table, and end-of-§10 checkpoint; the drafter-proxy sub-agent prompt itself lives in [`../_shared/follow-up-filing.md`](../_shared/follow-up-filing.md) (shared with `github-pr-evaluator`).**
...
**Native `blocked by` is a hard gate.** Step 2's `GATHER_ISSUE` returns the issue's native `blocked_by` list (GitHub issue dependencies; empty when none or when `deps_available` is false). If any blocker is **open** — typically a `question` issue routing an unresolved open question — the issue is **blocked**: classify it as the `blocked` response type (comment-only, §4) and surface "can't build until #N answers" instead of writing code. This is the structured half of the open-question contract; the issue's `## Open questions` section (marker `<!-- open-question-links:v1 -->`) is the human-readable half — a **tracked-dependency registry, not buildable scope and not a `## Definition of done` source**: read it for context (what's deferred and why), never implement its *registry bullets* or project DoD from them. This does **not** mean the section's OQs are all unbuildable: a `provisional-default` OQ's actual work lives in the normal `## Changes`/DoD (built on the stated provisional choice) and **is** in scope — the "don't implement" rule is about the registry entries, not the provisional scope the plan captures elsewhere. `scoped-out` parts, by contrast, live in `## Out of scope`, so §4.5's in-scope hard-block isn't triggered by them. See [`../_shared/open-question-links.md`](../_shared/open-question-links.md). (This is the same `blocked` comment-only path the distiller's `Blocked on:` line already feeds — native deps just make the block machine-detectable.)
...
| **Story** | Resolve like a normal feature/bug, but: (a) read the parent epic's `## Goal` and `## Background` as additional grounding; (b) target the parent epic's integration branch, not `main`; (c) PR title and body reference both the story and the parent epic. See "If the issue is a Story". |
...
- **Zero matches** → the integration branch hasn't been bootstrapped. Continue per "If the branch does not exist on origin" (epic flow) or the existing stop-and-redirect message (story flow).
...
**Check the integration branch.** Resolve `<branch>` per "Resolving the epic branch name" (discover by prefix; if zero matches, the bootstrap path will compute a fresh slug).

**Before running the rectification or bootstrap procedures, `Read references/epic-flow.md`** — it holds the drift-rectification runbooks (rebase / merge / conflict-handling sub-agent prompt / post-rectification), the bootstrap + legacy-recovery sequences, and the full-canonical-suite runbook. Forced into the chain here so these are in your working context regardless of where SKILL.md was truncated on initial load (SKILL.md exceeds the default Read cap; the forced Read is load-bearing for the epic-as-target flow).
...
This is the only path in the skill that rectifies epic-vs-main drift (by rebase or by merge — see "Choose strategy"). Story runs under this epic surface drift as an informational state-summary note but never act on it (see "If the issue is a Story" → "Determine the PR base").
...
```bash
# Commits the epic is behind main by (already computed in "Check the integration branch"; reuse).
...
Print the signals to the user as a table — `commits behind`, `prior main-merges`, `open story PRs` (with numbers + authors if non-empty), `overlapping files`. This is the audit trail for the "Choose strategy" decision.
...
**Execute the rectification.** With rebase or merge chosen, run the matching runbook in [references/epic-flow.md](references/epic-flow.md) → "Drift rectification": **Path A — Rebase** or **Path B — Merge**, then **Conflict handling** (shared, on conflict) and **Post-rectification** (re-baseline the changed epic HEAD, then post a fresh `Baseline established` comment). Then continue to "Classify the situation and act accordingly".

**If the branch does not exist on origin** → the epic infrastructure hasn't been bootstrapped yet. The epic-as-target run is the canonical place to do this — story runs deliberately stop and redirect here rather than bootstrap silently, so a missing step in the user's workflow stays visible. Run the **Bootstrap a new epic branch** sequence in [references/epic-flow.md](references/epic-flow.md) — a remote write: it pushes `epic/<N>-<slug>`, runs the green baseline, and posts the `Baseline established` comment.

**If the branch exists but the epic issue has no `Baseline established` comment** (epic predates this rule, or the comment was never posted) → run the **Legacy recovery** sequence in [references/epic-flow.md](references/epic-flow.md) to establish the baseline on the epic-branch HEAD. Without this comment, every story under the epic would otherwise stop and redirect back here — establishing it once unblocks the whole epic.
...
|---|---|
| **Open PR you authored, branch is yours** | Set up a worktree on the existing branch (see "Setting up the worktree"), read the full PR context (see "Reading the full PR context"), and continue from there. Skip the "create a branch" part of step 8. |
...
**Subagent delegation.** Static-checks commands are typically lints and codegen — small outputs, run inline. If any command in the block begins with `xcodebuild` (rare for static-only blocks, but possible), delegate to `apple-platform-build-tools:builder` per the convention in §P3.
...
| Story under an open epic | the epic integration branch | **Skip** if the epic-level baseline is still trusted (see "Trust state"). Re-run on the epic branch HEAD if a trust-decay event has fired. If the epic branch or its `Baseline established` comment is missing, the story flow stops and redirects to the epic-as-target bootstrap — it never bootstraps from the story flow. |
...
1. The epic issue has a "Baseline established" comment (see "Persistence").
2. Neither of the following events has occurred since that comment's date:
   ...
   - **A prior story under this epic landed under a baseline override** — detected by scanning the epic issue's comments for a "Baseline override" comment (see "Persistence") dated after the most recent "Baseline established" comment. If found, re-run on the epic branch HEAD.
...
*Baseline established* — posted whenever the skill runs a green baseline for this epic. Normally posted by the epic-as-target bootstrap (first run on the epic ticket); also re-posted from the story flow when a trust-decay event invalidates the inherited baseline (see "Trust state"):
...
- **Pre-existing failures unrelated to the issue.** Stop and surface to the user with the specific failures. Do not proceed on a red base. The point of the baseline is to attribute later failures correctly; that attribution falls apart the moment unrelated red is left in the tree, and it lets a PR ship over a broken codebase. Acceptable next moves, all chosen by the user: (a) detour first — file a follow-up issue per §P5 (urgency: `file-now`, type: `bug`), then open a separate detour PR that resolves it; resume this resolution once the detour merges and the baseline turns green; (b) explicit user override with a documented reason recorded in the PR body's out-of-scope notes. If this is a story under an open epic, also post a "Baseline override" comment on the epic issue (see "Persistence" in "Where to run it") so the next story under this epic knows to re-establish the baseline. Do not silently fix unrelated failures — that scope-creeps the PR and obscures what your change actually did.
...
Before writing code, **Read [`references/common-pitfalls.md`](references/common-pitfalls.md)** — the anti-patterns there apply throughout implementation and the review loop; the "Common pitfalls" section near the end of this file carries only their titles. Comment-only responses can skip the read — the few pitfalls that bear on them (comment routing, "don't open a PR for a question") are actionable from the title index alone.
...
- **Stick to the plan; fill gaps in the open, route reversals back, never work around silently.** When a plan was consumed at step 4.6, its locked decisions (`## Architecture decisions`, `## Changes`, `## Data model / schema impact`, `## Test plan`) are binding. The plan locks *decisions*, not every line — so your license is graded by what the discovery actually is:
  - **Below the plan's altitude** → settle it yourself, no record needed. This is line-level mechanics (local names, brace style, a single-use helper's internals, log/error wording) *and* work entailed by a locked decision (the call site a planned signature change forces you to touch; a test additive to `## Test plan`). Locked decisions are a **floor, not a ceiling** — adding an in-spirit test or touching an entailed file is not a deviation; only *reversing* a planned assertion or surface is.
  - **A genuine decision the plan left open** (it slipped past the planner's Step 7.5 sweep) → run a mini Step 7.5 at the seam. If precedent decides it unambiguously, settle it the way the planner would — read the call site, cite the precedent — **and record it in the PR body** (a `## Plan gaps settled` note, same shape as the `## Audit override` / `## Plan override` notes) so the deviation is visible to PR review and to the planner on its next revise. If precedent *can't* decide it and the choice carries a user-visible consequence (UX, performance, a layer assignment, future optionality — planner Step 6.5 territory), don't originate it — route it back to the planner in revise mode (the reversal case spells out the handoff mechanism).
  - **A reversal of a locked decision** (a planned API doesn't behave as assumed, a layer assignment can't hold, a data-model shape won't work) → **stop and route back to `github-issue-planner` in revise mode**: emit the Step 12 re-route handoff (its command is `re-plan #N — implementation invalidated a locked decision: <evidence>`), and **do not** call the `Skill` tool — re-routes never cross the session boundary (§12's Re-route rule). Never silently substitute your own approach.

  This mirrors how the §4.5 audit routes body problems back to the drafter: the plan is a verified, durable artifact, and a quiet workaround diverges the code from it, defeats the pr-evaluator's plan-adherence check, and loses the decision's provenance.
...
- **If this is a fresh PR**: push the branch and open a PR.
  ...
  **Multi-phase issues open as draft and carry a `## Phase tracker` block.** When §4.7 identified this as a multi-phase issue, two things change about the fresh-PR open:
  ...
  The tracker exists for the evaluator's eventual DoD check (it lets the evaluator see at a glance which phases shipped) and for human reviewers — the resolver itself only reads the plan's `## Phases` to decide routing in §11. On subsequent phases the existing-PR continuation path updates this block via `gh pr edit`.

  **Then project DoD onto the issue body, same as the existing-PR path.** Phase 1's `closes-dod` projects onto the issue's `## Definition of done` alongside the fresh PR's `## Phase tracker` initialisation. For a single-phase fresh PR, the single-phase fallback (see "DoD projection rule") ticks every top-level DoD bullet at this point. Same `gh issue edit --body-file` mechanism; same don't-abort-on-failure rule.

In both cases, capture the PR number/URL — you'll need it for the review loop.

#### DoD projection rule

Both §9 push paths (existing-PR continuation, fresh-PR open) project the just-shipped phase's `closes-dod` onto the issue body. The rule that follows specifies what to project and how to compose the annotation. Reconciliation on re-entry (§4.7) follows the same rule with the same inputs.

**Read [`references/dod-projection-rule.md`](references/dod-projection-rule.md)** before computing any projection diff — it specifies the annotation shapes and parser, the reconciliation source (`## Phase tracker` × §4.7's `closes-dod`), the single-phase fallback, operator-phase hybrid detection, the annotation format, the evaluator-sticky-veto rule, idempotent diff-only application, worked examples (A/B/C), and edge cases. Both the §9 push paths and §4.7 re-entry reconciliation apply the same rule with the same inputs.

### 10. Run the review loop (PRs only)

**Step 10 → Step 10.7 → Step 11 → Step 12 chain.** When the review loop exits cleanly (verdict approved + zero classified items), your next emissions in the same run are: (a) §10.7's operational state-refresh (`gh pr view <N> --json …` — mandatory, not advisory), (b) §11's summary block, (c) §12's `## Handoff`. All three are unconditional. The PR #416 / no-handoff #653 failure mode is: `Skill(review)` returns approved prose, the model treats that prose as a deliverable, and the turn ends. The primary structural anti-stop is §10.7's operational anchor — every sibling skill (pr-evaluator, drafter, planner) survives the same turn-boundary risk because they have operational tool calls between verdict and handoff. The two turn-boundary beats in §10 that require this protection: **after `Skill(review)` returns** (before step 2's sub-agent dispatch) and **after the §10 sub-agent returns** (before step 1 of the next iteration *or* before §10.7 / §11).
...
The sub-agent applies this rubric on every iteration. The rubric is documented at the section level so a reader can audit a sub-agent return against it and so the §10 sub-agent prompt can reference it by anchor.
...
- **Explicitly deferred** — reserved for items meeting **at least one** of these objective triggers. The reviewer's prose is evidence; the sub-agent re-checks. Soft framing alone never qualifies:
  ...
  - **PRD / scope explicit exclusion.** The item is outside the issue's stated scope as documented in the issue body, the parent epic's DoD, `docs/prd.md`, `docs/architecture.md`, `docs/constitution.md`, or `CLAUDE.md`. Soft scope arguments ("keeps the PR scope pure") don't qualify — the exclusion must be citable. **A citation only qualifies when the doc places the item *out* of scope. A doc that *requires* the behaviour (an invariant / MUST / "no X" rule) the diff violates is the opposite of an exclusion — that is the Grounding-violation bucket, not a deferral. The doc merely *mentioning* the topic is not an exclusion: e.g. an epic `## Out of scope` reading "tuning beyond the baseline" defers only the tuning and keeps the baseline in-scope.**
- **Cheap-fix override.** If an item meets an **Explicitly deferred** trigger but the fix is **≤ ~20 lines of edits on files this PR already modifies** and doesn't require new tests, new files in the diff, or scope expansion — address it in-loop anyway. Deferral exists to keep PR scope tight; trivial doc, comment, identifier, or formatting fixes aren't scope creep. The override does *not* apply to code changes that pull new files into the diff or that need a fresh test.
- **Decision required** — the suggestion touches architecture, breaks an API, or carries a tradeoff the user should weigh in on. Don't guess. Trip the sub-agent's "Decision required" guard rail (see the §10 sub-agent prompt): the sub-agent returns a `needs_decision` payload, the main loop asks the user via `AskUserQuestion`, and a fresh sub-agent resumes with the answer.
- **Grounding violation (in-scope) — never deferrable.** The item cites a diff that *violates* a documented constraint the issue/epic kept **in-scope**: a `docs/prd.md` requirement, a `docs/architecture.md` invariant, a `docs/constitution.md` directive, or a `CLAUDE.md` rule whose cited text *requires* the behaviour (not excludes it) **and** which the issue's / parent epic's `## Out of scope` does not exclude. This is **not** a follow-up. If addressable on this PR → reclassify as **Addressable** and fix it in-loop. If **not** addressable here (an integration PR of already-merged code, or a fix needing a plan/story) → trip the sub-agent's `grounding_violation` guard rail (see the §10 sub-agent prompt); the loop cannot exit clean until it is fixed or routed. The severity axis is "an in-scope documented constraint is violated" — *not* which doc it lives in (the pipeline has no constitution-over-architecture tier). Whoever classifies applies this bucket **before** filing any item as a follow-up — the §10 sub-agent normally, **and** this main loop when it reviews an integration PR with the editing sub-agent skipped. When a finding matches both this bucket and **Decision required**, this bucket wins — a hard block outranks a soft approval gate.
...
Exit the loop only when **all** are true: (a) the verdict is approved, (b) zero Addressable or Cheap-fix-override items remain after the sub-agent's re-classification, **and** (c) no Grounding-violation item remains unaddressed — a not-addressable one trips the `grounding_violation` guard rail, which exits the loop (re-route or abort) before this gate is reached, so by here any such item was fixed in-loop. The reviewer's own summary ("zero blocking", "all deferrable", "nothing addressable", "fast-follow only") does **not** make (b) true on its own — re-classify every listed item against the §10.4 rubric before exiting. The reviewer can frame items politely; the sub-agent decides whether they're addressable on this PR. If the re-classification finds any Addressable or Cheap-fix-override item, the loop continues — even when the reviewer's verdict line says "approved with zero blocking". A verdict like "approved with minor fixes" or "approved, with these nits" is the loop telling you it isn't done yet, not a green light to exit.

When you do exit, file each explicitly-deferred item as a follow-up issue per §P5 — urgency `file-now`, type chosen per the reviewer's framing (`bug` if the deferred item is a real defect, `incomplete-feature` if it's a half-built capability the reviewer flagged, `deferred-test` if it's a test the reviewer accepted should be skipped). The filed URLs land in this iteration's PR body `## Follow-ups` section before push and in the sub-agent's `items_filed_as_followups` return field. Procedural-only items (informational caveats with no tracked work) are not filed — emit them in `items_carried_as_procedural_notes` for §11's summary.

**A Grounding-violation item is never filed as a follow-up** — a follow-up tracks debt that *shipped*, and the hard-block stops the ship, so filing one would launder exactly the merge the block disallows. It resolves down one of three terminal paths instead:

1. **Fixed in-loop** (addressable on this PR) — reclassified as Addressable, the sub-agent makes the diff compliant and pushes; the next review pass no longer flags it and the loop exits normally.
2. **Re-routed to the planner** (not addressable here) — the `grounding_violation` guard rail fires and the run exits via the §11 re-route handoff (`re-plan #N — <evidence>`; for an epic the planner scopes the missing in-scope work as a story). The PR stays draft/unmerged; the missing work is tracked in the revised plan (or the story the planner scopes), never as a deferral.
3. **Abort** — the run stops, the PR stays draft/unmerged, and the finding is named in the §11 summary, the handoff, and the PR review comment. Nothing shipped, so there is no debt ticket to file (mirrors the §4.5 audit-abort artifact rule).
...
Run inside the sub-agent on every iteration that produced code changes (per the sub-agent prompt's step 7). Same three-step gate as §8, per §P3:

1. **Static checks** — run the `<!-- issue-resolver-fast-checks -->` block inline, fail-fast in declaration order. Outputs are small (lints, codegen, layer-import boundary checks); no need to delegate.
2. **Test selection** — spawn an `Explore` sub-agent with the prompt template from §P3.3. Substitute the worktree path, integration target, and the project's `<!-- issue-resolver-test-target -->` block. The sub-agent reads the diff, lists each declared target's directory, applies the heuristics, and returns two sections: `COMMAND:` (a ready-to-run shell command, or `(none)`) and `RATIONALE:` (one or two sentences). Capture the rationale in the iteration's PR-status comment so the user can audit the selection.
...
| §10.4 Grounding violation (in-scope) not addressable on this PR (the `grounding_violation` guard rail fired) | **Re-route → `github-issue-planner` (revise).** Reuse the §4.6 planner re-route shape; the handoff quotes the violated doc section + the in-scope evidence, and for an epic names the story that must build the missing in-scope work before the integration PR can land. |
...
**Phase-exhaustion check (multi-phase issues only).** Before deciding which multi-phase row of the §11 outcome rubric fires, check whether the plan's phases are exhausted:
...
**Close with the Step 12 Handoff.** After everything else in §11 has been emitted (Iteration test status, summary, Follow-ups split, worktree cleanup notes, story / epic reminders), end the run with the Step 12 handoff block defined in §12. Don't author a separate "Next step" line — the handoff *is* the next-step signal, and the §11 outcome rubric already classifies which rendering fires.
...
The full text of each pitfall lives in [`references/common-pitfalls.md`](references/common-pitfalls.md); step 8 ("Do the work") force-`Read`s it before any code or review-loop work. This index lists every pitfall by title, grouped by phase, so the catalog stays scannable without loading the full file — open the reference for the body of any that's load-bearing on the current run.

**Existing work & PRs**
- **Don't ignore in-progress PRs.**
- **Don't take over someone else's PR silently.**
- **Don't open a PR for a question.**
- **Don't skip the review loop.**
- **Don't open a single feature PR for an epic.**

**Docs & plan**
- **Don't implement code without grounding in project docs.**
- **Don't start non-trivial code work without a finalized plan.**
- **Don't re-plan when a plan exists.**
- **Don't silently deviate from a locked plan decision.**

**Baseline & test gates**
- **Don't skip the green-baseline check for the integration target.**
- **Don't silently fix unrelated pre-existing failures.**
- **Don't push code without running the §8 pre-push verification gate.**
- **Don't run the full unit + integration suite at the §8/§10.6 story gates.**
- **Don't fall back to zero tests when uncertain.**
- **§8 is the pre-push gate, not the dev inner loop.**
- **Don't push within the review loop without re-running the full suite.**

**Test-selection & build sub-agent**
- **Don't inline the test-selection reasoning in main context.**
- **Don't skip the rationale audit.**
- **Don't let the build subagent become a coder.**
- **Don't iterate small fixes when failures are sticky.**
- **Don't `rm` snapshot/golden files to force regeneration.**

**Review loop / §10**
- **Don't exit the loop just because the verdict says "approved".**
- **Don't drive `review` from inside the §10 sub-agent.**
- **Don't stop at either turn-boundary beat in §10.**

**Comment routing**
- **Don't read only the PR diff.**
- **Don't trust the issue title alone.**
- **Don't re-litigate decided questions.**
- **Don't post review feedback on the issue.**
- **Don't mis-route comments between issue and PR.**
- **Don't assume the issue is still relevant.**

**Worktrees**
- **Don't `git worktree add` a branch that's already checked out.**
- **Don't nest worktrees.**
- **Don't forget `.worktrees/` in `.gitignore`.**
- **Don't auto-clean worktrees.**
- **Don't skip worktree-setup on the create arm.**
- **Make worktree-setup idempotent — it runs on the reuse arm, too.**
- **Don't auto-clean a worktree without running teardown first.**

**Epics**
- **Don't target `main` for a story under an open epic.**
- **Don't let the epic branch drift silently — but only act from epic-as-target runs.**
- **Don't recompute the epic branch slug — discover it.**
- **Don't run epic baseline in the main checkout.**
- **Don't cold-rebuild the canonical suite on every re-run, and don't background it with a relative `cd`.**
- **Don't merge the integration PR without running the review loop.**
- **Don't ignore the body checkboxes when closing an epic.**
- **Don't restructure the epic body template.**
- **Don't edit a parent epic's body from inside a story-target run.**

**Follow-ups**
- **Don't hand-craft follow-up issue bodies.**
- **Don't conflate filing with capturing.**

**Handoff & DoD**
- **Don't omit the pr-evaluator handoff on PR outcomes.**
- **Don't hand off to `github-pr-evaluator` until the plan's last phase has shipped.**
- **Don't mark a multi-phase PR ready, and don't add `Closes #N` in reaction to shipping the last phase.**
- **Don't tick DoD bullets the phase's `closes-dod` doesn't claim.**
- **Don't re-tick DoD bullets the evaluator has rejected.**
