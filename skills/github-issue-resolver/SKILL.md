...
description: Investigate and resolve a specific GitHub issue end-to-end using the `gh` CLI. Use this skill whenever the user gives you a GitHub issue number, URL, or asks you to "look at", "work on", "fix", "implement", "resolve", "triage", or "respond to" an issue — including bug reports, feature requests, questions/discussions, and refactoring tasks. The skill reads the issue plus its full comment thread to determine the current state (separating stale early discussion from the latest decisions), checks for any existing open, draft, or prior PRs to avoid trampling in-progress work, figures out what kind of response the issue needs, does the work, and optionally posts a comment or opens a PR back to GitHub. For code changes, the skill opens (or continues) a PR and then loops with the `review` skill — posting review feedback as PR comments and addressing it — until review approves the PR. Reads project docs (`docs/prd.md`, `docs/architecture.md`, `CLAUDE.md`) when planning code changes to ground the implementation and surface tensions between the issue and project spec. Also recognises epic and story issues — for epics, audits child story progress, manages a long-lived `epic/<N>-<slug>` integration branch so `main` stays stable mid-epic, and orchestrates the integration PR + close ritual rather than opening a single feature PR; for stories under an open epic, targets the epic integration branch as the PR base and reads the parent epic's Goal/Background as additional grounding. Use this even if the user only casually mentions an issue ("can you take a look at #423?") or an epic ("what's left in the auth epic?") — don't try to handle GitHub issues without it.
---
...
Format matches the existing `pr-evaluator-health-checks` / `issue-resolver-fast-checks` blocks: one Markdown list entry per command, backtick-quoted command followed by ` — ` and a short human description. Order matters — commands run in declaration order. **Setup is fail-fast** (the first non-zero exit stops the run); **teardown is best-effort** (a failure is logged but doesn't block subsequent teardown commands or the worktree removal that follows).
...
4. Apply per-target widening rules from the config:
   ...
   - If a target declares a fallback as `none`, do not widen for that target —
     ...
     run cover the gap.
5. Pure-docs / comment-only diffs (only `*.md`, `docs/`, code-comment changes) →
   COMMAND: (none).
...
This mirrors `github-pr-evaluator` §5.5. The targeted-tests strategy means each invocation runs only the selected suites, so the build log per iteration is much shorter than a full-suite run anyway — but the delegation layer is still useful and keeps the convention consistent across both skills.
...
**What to run.** The static-checks list from the `<!-- issue-resolver-fast-checks -->` block. **No tests run at the baseline gate** — there's no diff yet vs the integration target, so test selection (§10.6) would be a no-op. The full canonical suite is `github-pr-evaluator`'s sole responsibility; this skill doesn't run it at any gate. The CI on the integration branch is the authoritative answer to "is `main` green?"; the baseline gate's job is narrower: confirm the *toolchain* is healthy on this developer's machine before code changes start.

If neither block exists in the project's `COMMANDS.md` / `CLAUDE.md`, fall back to running the `<!-- pr-evaluator-health-checks -->` block (the canonical suite) inline — accept the cost on a project that hasn't declared a static-checks subset. If even that block is absent, ask the user; don't invent commands.
