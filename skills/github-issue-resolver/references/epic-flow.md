...
- **Any re-run** (the first run's result was lost or partial, or you're re-running specific failures) → `build-once` **once**, then `retry-without-rebuild` (narrowed to only the failures via the wrapper's targeted-run syntax — e.g. append `-only-testing <Suite>/<test>`, or pass `<path>:<line>` to `bin/rails test`). Do **not** re-issue `full-suite`.

The reason is wall-clock: on a compiled stack a plain `<wrapper> test` recompiles the whole app target on every invocation, and that cold build — not the tests — dominates the time (on interpreted stacks the three labels collapse to one command and there's no rebuild to avoid). Re-paying that build on each retry is what produced the past hang. If the block is absent, fall back as described in SKILL.md §P3.1 (pr-evaluator `full-suite-command`, then prose + ask) and tell the user retries will cold-rebuild on compiled stacks until the block is declared.
