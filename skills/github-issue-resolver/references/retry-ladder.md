...
The pre-push verification gate (§8 before the first push, §10.6 after each round of review feedback) treats "tests must be green before push" as a goal, not a license to retry indefinitely. Without a cap, a complex integration-test failure can spiral into a sequence of small fixes — tweak, re-run a slow (10–20 minute) integration suite, tweak, re-run, repeat — that burns hours of wall-clock time and produces nothing the review loop couldn't have surfaced in the first place. The cost compounds because each iteration re-pays the suite's fixed setup overhead — a cold build, a booted simulator, a provisioned server/database, whatever the stack pays once per run — and small fixes on a shallow read of a failure usually don't help anyway.
...
A "cheap fix" is a small, narrowly-targeted edit (typo, a missing test identifier, off-by-one, an obvious binding/wiring mistake) on the immediate failure mode. A "deep fix" is a structural change informed by the research breakpoint — e.g. restructuring a SwiftUI gesture or lifting view state, reworking a Rails `before_action` chain or a shared partial, etc.

## Triage unrelated failures: check for a known issue before spending the fix budget

A red gate run does not always mean *your change* broke something. Targeted selection deliberately widens beyond the diff — the integration blast-radius rules and the broad-change-fallback pull in tests that don't transit your edit at all — and those tests can be red for reasons that predate this branch: a flaky interaction, an environmental timing bug, a genuinely separate defect someone has already filed. The expensive way to find that out is to revert your diff and re-run; it works, but it re-pays the suite's setup cost (cold build, simulator boot, server spin-up) to answer a question GitHub may already answer for free. So triage cheaply first, before the ladder spends a single fix attempt.
...
- **Seemingly-unrelated failures** — pulled in only by widening (integration blast-radius rule, step 5; broad-change-fallback) and referencing no symbol or file in your diff. These, and only these, are the triage candidates.
...
1. **Read each failing test's full output** — the assertion message, the stack frame, and the relevant runner/diagnostic log lines (simulator logs, server logs, etc.). Not just the one-line summary the test runner prints.
2. **For integration tests, capture the stack's richest failure context** — e.g. XCUITest `app.debugDescription`, a Capybara `page.html` / screenshot — from inside the failing test and read the dump. If the project's CLAUDE.md mandates such a capture (the Swift convention mandates `app.debugDescription` on element-lookup timeouts), restate and follow that mandate here. The captured surface usually points at the cause directly — for a UI test: a collapsed parent, hidden element, dropped identifier, overlay absorbing children; for a request/system spec: a redirect chain, a missing record, an authorization filter rejecting the request.
3. **Spawn one `Explore` sub-agent** with: the failing test files, the source they exercise, the recent diff (`git diff <integration-target>...HEAD`). Have it return three things, in order:
   ...
   - What code paths the failing tests transit, including any indirection (e.g. SwiftUI gesture → focus → state → re-render, or Rails request → `before_action` → service → view render)
...
2. **Defer the failing tests with linked issues.** File one follow-up issue per failure (or one umbrella issue if the failures share a root cause) via the sub-agent protocol in "Follow-up issue tracking" — urgency `file-now`, type `deferred-test`. The filed URLs become `// TODO(#NNN)` markers and skip annotations (`XCTSkip("Deferred to #NNN — <reason>")`, Minitest/RSpec `skip "..."`) before push. Push the rest green. Best when the failure is a real structural problem that needs more design than fits this PR.
3. **Restructure.** Abandon the current approach. Return to a planning conversation, with the research-breakpoint findings as input, and propose a different shape. Best when the deep fix revealed that the original approach itself was wrong (e.g., a SwiftUI gesture that fundamentally fights its parent's gesture system, or a Rails callback chain that can't express the transaction boundary the feature needs).
