...
2. **Identify what changed since the plan was written** — re-walk the thread for newer decisions, re-grep the codebase **at `plan_ref`** (`git grep <pattern> <plan_ref>`) for symbols the plan names that may have drifted, re-read any external sources. **Re-check open questions:** an OQ the prior plan planned around that the source register now marks `Decided`/`Applied` (or whose companion `question` is closed/answered) is no longer deferred — **fold the now-decided scope into the plan** (`## Changes`/`## Test plan`) as a SOFT change and drop its `## Open questions` entry; if the issue was natively `blocked by` it, note that the block can be removed. A newly-opened OQ adds a plan-around entry. The planner still never resolves an open OQ itself.
