...
2. **Resolve the integration target.** Derive `plan_ref` per the Step 4.5 table: discover the `epic/<N>-<slug>` branch (`git ls-remote --heads origin "epic/<N>-*"`), `git fetch --quiet origin <branch>`, and capture its **current HEAD** — that `epic/<N>-<slug>@<short-sha>` is this story's `plan_ref` (or, when the story already has an open PR, that PR's `headRefName` HEAD, which wins per the table's open-PR row).
