...
2. **Identify what changed since the plan was written** — re-walk the thread for newer decisions, re-grep the codebase **at `plan_ref`** (`git grep <pattern> <plan_ref>`) for symbols the plan names that may have drifted, re-read any external sources.
