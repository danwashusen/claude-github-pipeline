...
**Detect open-question dependencies.** Parse the issue body's `## Open questions` section (located by `<!-- open-question-links:v1 -->`) if present, and read the issue's native `blocked_by` from the `GATHER_ISSUE` result (the `blocked_by` list / `deps_available` flag). Record each entry's OQ id, gated scope, disposition (`scoped-out` / `in-scope (blocked)` / `provisional-default`), and companion `question: #N`. Also note any **in-scope PROVISIONAL marker** you hit while grounding (Step 5) that the issue kept in-scope — recognized via the detection contract [`../_shared/open-question-detection.md`](../_shared/open-question-detection.md) (the `<!-- drafter-open-question-markers -->` block or the heuristic cues). These are decisions a human must make — **not** design choices you resolve from precedent. You read this contract; you never rewrite the body section. See [`../_shared/open-question-links.md`](../_shared/open-question-links.md). How they shape the plan: Step 7.5 (they're not banned hedges), Step 7's `## Open questions` plan section (plan-around vs re-route), and Dimension 10 (Step 8).

When a detected entry's companion is recorded `(not filed)`, re-run the tracker de-dup search from [`../_shared/open-question-detection.md`](../_shared/open-question-detection.md) §Matching yourself before copying it into the plan — one cheap `gh issue list --repo <owner/repo> --state all --label question --search "<query>"` call. A companion may have been filed after the source doc or issue body was last edited, and Dimension 10 re-verifies this same claim later anyway; catching it here (the same cheap-in-loop-first pattern Step 7.5 uses for hedges) saves a reviewer round-trip.
...
## Step 4.5: Derive the plan ref (`plan_ref`)

Every doc and code read from here on is grounded at one git ref — `plan_ref`, the **integration target the plan is built against** — never the orchestrator's working tree. The working tree is typically the main checkout sitting on `main`, but the issue's integration target may be a different branch: an epic branch, or the head of an open PR mid-revise. Reading the working tree instead of `plan_ref` grounds the plan on stale docs and code — citing `docs/architecture.md` as it reads on `main` when a predecessor story already moved a layer rule on the epic branch, and running the Step 6 deviation / constitution gate against the wrong copy. This is the failure class the resolver's §4.5 audit was built to prevent (its Epic #154 incident: the audit grepped `main` and missed every epic-branch symbol).

Derive `plan_ref` from issue context:

| Issue context | `plan_ref` |
|---|---|
| Regular issue (bug / feature / incomplete / refactor / multi-phase), no open PR | `origin/main` |
| Epic-as-target | `origin/epic/<N>-<slug>` HEAD — branch name via `git ls-remote --heads origin "epic/<N>-*"` |
| Story under an open parent epic | `origin/epic/<parent-#>-<slug>` HEAD — the parent epic's branch, discovered alongside the story-type determination (Step 3) |
| A plan run where an open PR for this issue already exists (the normal **revise** case) | the PR's `headRefName` HEAD — the work-in-flight branch the resolver will continue on, carrying the shipped phases and any doc edits not yet on the base |
| Story with no parent epic, or parent epic is closed; epic bootstrap (zero `git ls-remote` matches) | `origin/main` |

When more than one row applies — a story under an open epic that *also* has an open PR — the open-PR-head row wins: that head is a strict superset of the epic branch (epic HEAD plus the story's shipped commits), and it is what the resolver continues on. The epic/story and revise rows are *materialized* where their branch first becomes known — the epic branch at Step 11 / Just-in-time story planning, the PR head in Revise mode — but the rule lives **only** here; those sections cite this table rather than restating it. Before any ref read, fetch the chosen branch so the remote-tracking ref resolves: `git fetch --quiet origin <branch>` (e.g. `main`, the epic branch, or the PR head).

**Read ref vs recorded ref.** The table gives `plan_ref` — the **read ref**: the `origin/`-prefixed remote-tracking ref every grounding read runs against (`git show <plan_ref>:<path>`, `git grep <pattern> <plan_ref>`). The plan footer (Step 7) and the handoff `Grounding:` line instead record the **integration branch** `plan_ref` points at, as `<plan-ref>@<short-sha>`: `origin/main` for the default branch, or the bare `epic/<N>-<slug>` / `<headRefName>` with the `origin/` prefix dropped (this is also the resolver's PR base). Same ref, two renderings — read at the remote-tracking form, record the branch.

## Step 5: Research and ground the approach

Read the project docs that constrain the work **at `plan_ref`** (Step 4.5), not the working tree — a doc reads differently on the epic branch or an in-flight PR head than on `main`, and grounding against the wrong copy is the bug this skill's ref discipline exists to close. List which docs exist at the ref, then read each through git — don't plan against memory:

```bash
git ls-tree -r --name-only <plan_ref> -- docs/ CLAUDE.md   # which docs exist at the ref
git show <plan_ref>:docs/architecture.md                    # read each that exists, per path
```

Read each that exists at `plan_ref` via `git show <plan_ref>:<path>`; follow `@`-references the same way (`CLAUDE.md` pulls in `docs/constitution.md` — read it as `git show <plan_ref>:docs/constitution.md`). A doc absent at the ref is simply not present. This mirrors the plan-reviewer prompt, which already reads docs at `plan_ref`. The split matters:
...
`<plan-ref>@<short-sha>` records the integration branch `plan_ref` points at (Step 4.5's read-vs-record note) — the integration target the plan was built against — giving the resolver's step-4.6 currency check both the branch and the commit: `origin/main` for a regular issue, the **full, un-truncated** `epic/<N>-<slug>` branch for an epic or a story under an epic, or the open PR's `headRefName` for a revise. Don't elide the branch to `epic/<N>-…` — it is also the resolver's PR base. Pinning the footer to the PR head on a revise is safe for the currency check: a revise-with-open-PR is a resolver **continue**, where the §4.5 audit (and so the §4.6 dimension-7 currency check) is skipped, so a PR-head footer never reaches a firing check; and if that PR is later abandoned, a fresh resolver run grounds at `origin/main` and the currency check correctly re-routes the now-stale plan.

**See [`references/plan-schema.md`](references/plan-schema.md) for the verbatim `<!-- implementation-plan:v1 -->` schema** — the ordered section headings the resolver and pr-evaluator parse (`## Approach`, `## Doc grounding`, `## Architecture decisions`, `## UI decisions`, `## Changes`, `## Data model / schema impact`, `## Test plan`, `## Coverage gap`, `## Phases`, `## External sources consulted`, `## Deviations from project docs`, `## Risks & watchpoints`, `## Open questions`) and the closing provenance footer. Use it verbatim; omit optional sections when empty, never pad. The `## Phases` per-key semantics (`kind` / `ships` / `closes-dod` / `deliverable` / `depends-on`) are detailed below.
...
### Open questions the plan plans around

When Step 3 detected open-question dependencies, add the plan-schema `## Open questions` section (see [`references/plan-schema.md`](references/plan-schema.md)) recording each: `gates`, `question: #N`, and a `treatment`:

- **planned-around** — the unblocked scope is planned fully in `## Changes` / `## Test plan`; the gated part is deferred (it was `scoped-out` at draft, or belongs to the follow-up). The deferred part does **not** appear in `## Changes`.
- **recorded-blocked** — the `in-scope (blocked)` part the plan cannot specify until #N answers. Named here, absent from `## Changes` / `## Test plan`. The issue's native `blocked by` already holds the resolver/evaluator until the question is answered.
- A **provisional-default** OQ is planned as a normal decision **plus** a `## Risks & watchpoints` entry naming the provisional choice and its retirement condition (`#N answered`), and listed here for visibility.

Never silently resolve an OQ — that's the whole point (Step 7.5 carve-out).

**Re-route if the whole plan is blocked.** If every plannable part is gated by an unanswered OQ (nothing can be built without a human decision), do **not** post a hollow plan — stop and emit the planner→answer-the-question re-route (Step 12). Boundary case: if only a trivial slice or the coverage-gap regression test is unblocked, treat it as total-block and re-route (a plan that builds nothing the issue actually asked for is not a plan). If no companion question exists yet, the re-route points at the drafter to file one first.
...
**Carve-out — a tracked open question is not a hedge.** Distinguish two things that look alike: a **hedge** is the planner failing to decide something *the codebase or docs could decide* (still banned — resolve it via 1/2/3). A **tracked open question** is a decision *a human must make* that the source already flagged open (Step 3's open-question detection) — you must **not** resolve it from precedent or the Decision gate, because it isn't yours to answer. Record it in the plan's `## Open questions` section (Step 7) with the OQ id, the companion `question: #N`, and a `planned-around` / `recorded-blocked` treatment; the gated scope is planned around, not guessed. A phrasing that reads like a hedge but is attributed there — naming its OQ id and companion question — legitimately survives. An **unattributed** punt still fails the gate.

**Exit gate.** After the sweep, no hedge phrasing may remain anywhere in the plan body. The phrasings that may legitimately survive are: concrete decisions with precedent citations, deviations recorded in `## Deviations from project docs` with their agreed date, user-locked decisions with `[user decision <date>]`, watchpoints in `## Risks & watchpoints`, and tracked open questions attributed in `## Open questions` (OQ id + `question: #N` + a plan-around/recorded-blocked treatment). If a hedge slips through, the dimension-4 reviewer will catch it at step 8 — but that's a passive backstop, not a substitute for this sweep.
...
**Dimensions.** Ten, defined in the prompt. Pass the subset that applies:
...
**Dimension 10 — open-question integrity.** Fires when the plan has an `## Open questions` section. The reviewer BLOCKs when: a `question: #N` doesn't resolve to a real `question`/`audience:*` issue; an OQ the plan planned around is now **resolved** in the tracker (its companion question closed, or answered-in-thread — the tiered read in [`../_shared/open-question-links.md`](../_shared/open-question-links.md) §"Status is the tracker's"), so the plan must build it, not defer; the plan silently resolves the OQ (decides the gated subject with no `[user decision <date>]` and no `## Open questions` treatment — the mirror of the drafter's frozen-undecided check); a `planned-around`/`recorded-blocked` part also appears in `## Changes`/`## Test plan`; or a `question: (not filed)` claim turns out to have a matching companion already in the tracker (a de-dup search miss). The full check is in `references/plan-reviewer-prompt.md`.
...
**Open questions at the epic grain.** If the epic's source carries open questions (Step 3 detection), add the epic plan's `## Open questions` per Step 7 — OQs that gate the epic as a whole or specific stories. A story that is *fully* gated by an unresolved OQ is scoped out of `## Story breakdown` (it becomes a follow-up story once the question is answered — the story-grain analog of scope-out); a story merely *touched* by an OQ carries the dependency into its own just-in-time plan.
...
2. **Resolve the integration target.** Derive `plan_ref` per the Step 4.5 table: discover the `epic/<N>-<slug>` branch (`git ls-remote --heads origin "epic/<N>-*"`), `git fetch --quiet origin <branch>`, and capture its **current HEAD** — `plan_ref` for grounding reads is `origin/epic/<N>-<slug>`, recorded in the footer / `Grounding:` line as `epic/<N>-<slug>@<short-sha>` (or, when the story already has an open PR, that PR's `headRefName` HEAD wins per the table's open-PR row).
...
4. **Ground and draft this story.** Run Steps 5–7 against `plan_ref` (the epic branch HEAD, or the open story PR head per Step 4.5): read code **and docs** at that ref (Step 5), ground every decision, and draft the story plan with the standard schema, the `**Epic:** #<N>` backlink first after the marker, and a `## Epic contract` section naming what it `Delivers` (matching the epic's `## Story contracts`) and `Consumes` (each already recorded in the epic's `<!-- epic-delivery-log:v1 -->` comment), `[epic-plan: #<N>]`-cited.
5. **Verify (Step 8) with dimensions 1, 2, 3, 4, 6, 8** (plus **10** when this story plan has an `## Open questions` section) — pass the parent epic plan as `<<epic_plan>>` and its delivery-log comment as `<<epic_delivery_log>>` so Dimension 8 checks this story's contract against the epic's `## Story contracts` and the delivery log. A Dimension-8 BLOCKER that traces to a wrong *epic* contract is the same **feedback edge** as step 3 — stop and re-route to the planner on the epic in revise mode (don't reshape this story to fit a wrong contract, and don't run the epic revise inline). **If this story is itself fully gated by an unresolved open question** (nothing plannable until a human answers), re-route via the planner→answer-the-question handoff (Step 12) instead of authoring a hollow story plan — the same total-block rule as Step 7; a story merely *touched* by an OQ is planned-around and carries an `## Open questions` entry.
...
Revise mode applies here too: if this story already has a plan, refresh it per Revise mode against `plan_ref` from the Step 4.5 table — the open story PR head if one exists (a superset of epic HEAD), else today's epic HEAD — with the same Dimension-8 check.
...
The snapshot lines are mechanical — the issue/Epic number and title are already in hand from Step 2's `GATHER_ISSUE`, the plan-comment URL was captured at Step 10, and child-story state for an Epic is from Step 11's `GATHER_EPIC`. When Step 4 ingested a research dossier, the `Issue:` / `Story:` line also carries `research: ✓ (<dossier-url>)` (omitted when no dossier exists), per `../_shared/handoff-format.md`. The `Grounding:` line opens with `read at <plan-ref>@<short-sha>` (the same ref/sha the plan footer records, per Step 4.5), then summarizes the docs (with §refs) the plan was built on — lifted from the `## Doc grounding` section you just authored — plus any `## External sources consulted` entries as a `· external: …` segment; omit the line on the trivial-change and knowledge-gap exits (no plan posted) and when the plan has no `## Doc grounding`. The `Why:` line is judgment — describe what the next session will do.

**Before composing the handoff, `Read references/handoff-renderings.md`** — it holds the worked `## Handoff` shapes the planner emits: single-issue plan posted, epic plan posted (contracts pinned; stories planned just-in-time), just-in-time story plan posted, epic plan with stories not yet filed (route to the drafter), trivial change (no plan), knowledge-gap re-route to the researcher, plan posted with planned-around open questions (forward + free-form `Open questions:` line), the open-question total-block re-route (terminal-style, `plan: ✗`), and revise-mode refreshed. It's a progressively-disclosed reference — not auto-loaded with this skill — so the forced Read is what guarantees the shapes are in your working context before you emit; without it the handoff may be written from memory and drift from the closed-set shapes. Each carries the closed-set state-marker vocabulary from [`../_shared/handoff-format.md`](../_shared/handoff-format.md); fill the snapshot from the data Step 12 lists above.
...
1. **Fetch the existing plan + downstream state.** Fetch the existing plan comment (step 2) and the issue + thread (note the plan's recorded SHA). Also check whether a draft PR exists for this issue (`gh pr list --search "linked:#<N>" --json number,state,isDraft,body,headRefName`) — if so, fetch its body and parse the `## Phase tracker`, **and set `plan_ref` to its `headRefName` HEAD per the Step 4.5 table** (`git fetch --quiet origin <headRefName>`), so the re-plan grounds on the shipped phases and any doc edits the resolver has already landed on that branch rather than on the base. With no open PR, `plan_ref` stays the base from the table (`origin/main`, or the epic branch for a story). Fetch the issue body to capture the current state of its `## Definition of done` annotations. Hold all four (old plan, issue body, PR body+tracker, captured annotations) in working context for the diff.
2. **Identify what changed since the plan was written** — re-walk the thread for newer decisions, re-grep the codebase **at `plan_ref`** (`git grep <pattern> <plan_ref>`) for symbols the plan names that may have drifted, re-read any external sources. **Re-check open questions:** an OQ the prior plan planned around whose companion `question` is now **resolved** — closed, or `resolved-in-thread` via the tiered read (see [`../_shared/open-question-links.md`](../_shared/open-question-links.md) §"Status is the tracker's"), not a doc register field — is no longer deferred: **fold the now-decided scope into the plan** (`## Changes`/`## Test plan`) as a SOFT change and drop its `## Open questions` entry; if the issue was natively `blocked by` it, note that the block can be removed. A newly-opened OQ adds a plan-around entry. The planner still never resolves an open OQ itself.
3. **Re-run steps 5–8** against `plan_ref` (Step 4.5), focused on what changed (don't re-derive untouched sections from scratch).
