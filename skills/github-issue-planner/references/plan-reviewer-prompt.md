...
10. **Open-question integrity** *(fires when the plan has an `## Open questions` section — see `<<dimensions>>` routing)*. The plan may depend on human-owned open questions it must **not** resolve itself. Read the plan's `## Open questions` against the issue body and the companion `question` issues (the registry of record — the tracker, not a doc register field):
   ...
   - **The plan does not silently resolve the OQ.** No `## Architecture decisions` / `## Changes` entry may decide the gated subject without either a `[user decision <date>]` citation or an `## Open questions` `planned-around`/`recorded-blocked`/`provisional-default` treatment. Silently baking an answer to a human-owned question is a BLOCKER — the mirror of the drafter's frozen-undecided check, applied to the plan. A `provisional-default` treatment is not silent by definition — it decides the subject openly, in `## Open questions`, with a named retirement condition — so a decision covered by one is not a violation of this bullet.
   - **Deferred scope stays out of the build.** A `planned-around` or `recorded-blocked` part must not also appear in `## Changes` / `## Test plan` as built scope (consistency with scope-out). This does not apply to `provisional-default`: that treatment is built now by design, so it belongs in `## Changes` / `## Test plan`.
   - **A `question: (not filed)` claim is checked against the tracker, not taken on faith.** For any `## Open questions` bullet whose `question:` field is `(not filed)`, run the de-dup search from `open-question-detection.md` §Matching: `gh issue list --repo <<repo_owner>>/<<repo_name>> --state all --label question --search "<query>"`, using the OQ's tracker id when it has one or its distinctive topic keywords when detection was heuristic. If a candidate comes back, `Read` its body to confirm it's the same question, not just a keyword hit. A confirmed match is a BLOCKER: name the missed issue number and quote both the `(not filed)` claim and the matching issue's title/body line that confirms it. An ambiguous candidate is a SUGGESTION naming it for the planner to check — never auto-resolved.

   Evidence for a dimension-10 finding: quote the `## Open questions` entry and the conflicting `## Changes`/`## Architecture decisions` line, or the companion question's state/thread finding, or name the dangling `question: #N`, or — for a missed `(not filed)` claim — the matching issue's number and a quoted line from its body confirming it's the same question.
...
- Whatever a dimension's own "Evidence for a dimension-N finding" sentence names in addition to these three (e.g. dimension 10's matching-issue number + confirming body quote) — those sentences are a per-dimension supplement to this list, not a restatement of it.
