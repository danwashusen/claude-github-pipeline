...
| `/github-pipeline:resolver` | Implements one issue against its verified plan **in the worktree you opened with `workspace-open`** (the session starts inside it and is verified there), opens/continues a PR, projects Definition-of-done ticks as phases ship, and loops with code review until approved — reviewing each non-final phase's own delta against the plan, and the whole diff in depth on the final phase. |
